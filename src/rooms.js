...
import { giftsObj_mobile, giftsObj, giftsInModal } from "./data/data";

const mobile_size = 820;
const desktop_size = 1100;
let defaultSizeRoom;
...
class RoomsApp {
  constructor() {
        ...
        item.addEventListener("click", (e) => {
          ...
          if (
            ...
            if (first_visit) {
              ...
              let elOnboardingWrapperGift = getElement(
    ...
    function getIndexElementFromCollection(el) {
      let currentIndex = String(el.parentNode.parentNode.classList).substr(
        "gift gift".length,
      );
      return currentIndex;
    }
    ...
    function deleteGiftFromRoom() {
      getArrayElements(".js-del-gift").forEach((item) => {
        item.addEventListener("click", function () {
          ...
          const tempGifts = Array.from(
            document.querySelectorAll(".gifts__item"),
          );
    ...
    function createBlock(num) {
      ...
      const showHideButtonsGift = function () {
        if (time2_Ms - time1_Ms < 150) {
          ...
          const blockButtons =
            this.parentNode.querySelector(".js-gift-buttons");
    ...
    if (getElement(".js-rooms")) {
      ...
      function moveRoom() {
        function moveAt(pageX, pageY) {
          ...
          elCurrentRoom.setAttribute(
            "style",
            "margin: initial; transform: scale(" +
              scale +
              ") translate(" +
              valueTranslateX +
              "px," +
      ...
      function goRoom() {
        elChoice.addEventListener("click", (e) => {
          if (
            ...
            let classes;
            if (classes1.includes("js-go")) {
              classes = classes1;
            }
            if (classes2.includes("js-go")) {
              classes = classes2;
            }
            ...
            if (!navigator.userAgent.includes("Fire")) {
              elBlockRange.style.width =
                getElement("body").offsetHeight - topValue * 2 + "px"; //всему блоку с ползункои присваиваем высоту подолжки модалки т.к. она всегда по высоте во весь экран
            } else {
              elBlockRange.classList.add("isMoz");
              elBlockRange.style.height =
                getElement("body").offsetHeight - topValue * 2 + "px"; //всему блоку с ползункои присваиваем высоту подолжки модалки т.к. она всегда по высоте во весь экран
              elBlockRange.style.top =
                (availableHeight - elBlockRange.offsetHeight) / 2 + "px";
            }
            ...
            if (first_visit) {
              ...
              if (screen.width > 820) {
                getElement(".js-onboarding-content").style.left =
                  elBlockRange.getBoundingClientRect().x +
                  elBlockRange.offsetHeight +
                  "px";
                getElement(".js-onboarding-content").style.top =
                  elBlockRange.getBoundingClientRect().y + "px";
                getElement(".js-onboarding-content-gift").style.left =
                  elMenu.getBoundingClientRect().x - elMenu.offsetWidth + "px";
                getElement(".js-onboarding-content-gift").style.top =
                  elMenu.getBoundingClientRect().y + elMenu.offsetHeight + "px";
                getElement(".js-onboarding-content-lk").style.left =
                  elMenu.getBoundingClientRect().x -
                  elMenu.offsetWidth / 4 +
                  "px";
                getElement(".js-onboarding-content-lk").style.top =
                  elMenu.getBoundingClientRect().y +
                  elMenu.offsetHeight +
                  50 +
                  "px";
              }
      ...
      elRange.addEventListener("input", () => {
        ...
        if (isMoveRoom) {
          ...
          elCurrentRoom.style.transform = `margin: auto; scale(${
            parseInt(elRange.value, 10) / 100
          })`;
        }
      });
        ...
        item.addEventListener("click", () => {
          ...
          if (cangedValueScale >= 100 && cangedValueScale <= 200) {
            if (isMoveRoom) {
              ...
              elCurrentRoom.style.transform =
                "scale(" + elRange.value / 100 + ")";
            }
          }
        });
      });
